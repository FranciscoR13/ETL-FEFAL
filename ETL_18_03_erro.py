...
# Coleções onde estão os concelhos e freguesias
concelhos_collection = db["CodPostais"]

# Obter concelhos únicos
concelhos_list = list({
    clean_text(doc["province"])
    for doc in concelhos_collection.find({"province": {"$ne": None}}, {"_id": 0, "province": 1})
})

# Obter freguesias únicas
freguesias_list = list({
    clean_text(doc["community"])
    for doc in concelhos_collection.find({"community": {"$ne": None}}, {"_id": 0, "community": 1})
})
...
if col_submissao:
    col_index = df.columns.get_loc(col_submissao) + 1
